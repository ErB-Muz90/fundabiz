...
export function SMESidebar({ isOpen, onClose }: SMESidebarProps) {
      ...
      >
        <div className="flex items-center justify-between px-6 py-5 border-b border-gray-200">
          <Link href="/overview" className="flex items-center gap-2">
            ...
            <span className="text-xs bg-blue-50 text-blue-700 px-2 py-0.5 rounded-full font-medium">SME</span>
          </Link>
          ...
          {links.map((link) => (
            ...
            >
              <link.icon className={clsx(
                'w-5 h-5',
                isActive(link.href) ? 'text-brand-600' : 'text-gray-400'
              )} />
              {link.label}
            </Link>
          ))}
